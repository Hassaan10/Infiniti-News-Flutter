...
class HeadlinesBloc extends Bloc<NewsEvent, NewsState> {
  final NewsRepository _newsRepository;
  HeadlinesBloc(this._newsRepository) : super(const NewsState()) {
    on<NewsFetched>(_onNewsFetched);
  }
  Future<void> _onNewsFetched(NewsFetched event, Emitter<NewsState> emit) async {
    try {
      if (state.status == NewsStatus.loading) {
        final result = await _newsRepository.fetchHeadlines();
        if(result.isSuccess) {
